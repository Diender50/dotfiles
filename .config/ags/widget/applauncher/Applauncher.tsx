...
export default function Applauncher() {
  ...
  function launch(app?: AstalApps.Application) {
    if (app) {
      ...
      // Détacher le processus pour éviter qu'il soit tué avec l'app launcher
      const command = app.executable || app.name
      execAsync([
        "sh", 
        "-c", 
        `setsid ${command} </dev/null >/dev/null 2>&1 &`
      ]).catch((error) => {
        console.error(`Erreur lancement ${app.name}:`, error)
        // Fallback avec la méthode standard si setsid échoue
        try {
          app.launch()
        } catch (fallbackError) {
          console.error(`Fallback failed for ${app.name}:`, fallbackError)
        }
      })
    }
  }
